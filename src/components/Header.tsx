...
import { DONATIONS_LIVE, NAV_LINKS } from "@/lib/constants";
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-neutral-200 bg-white/95 backdrop-blur">
      <div className="bg-brand-900 py-1.5 text-center text-xs text-brand-100">
        Civic, humanitarian, and legal education for every student, in every community.
      </div>

      {/* Logo row — its own row so nav/CTAs can never overlap it */}
      <div className="bg-brand-500">
        <Container className="flex items-center justify-between py-3 lg:justify-center lg:py-4">
          <span className="w-10 lg:hidden" aria-hidden="true" />
          <Link href="/" aria-label="AVENUE JAM home">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/brand/avenuejam-bar.png" alt="AVENUE JAM" className="h-8 w-auto sm:h-10 lg:h-12" />
          </Link>
          <button
          ...
          <nav className="flex items-center gap-1" aria-label="Primary">
            {NAV_LINKS.map((link) => (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 rounded-md px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-brand-50 hover:text-brand-900"
                >
                  {link.label}
                  {link.children && (
                  ...
                  <div className="absolute left-0 top-full w-64 rounded-lg border border-neutral-200 bg-white p-2 shadow-lg">
                    {link.children.map((child) => (
                      <Link
                        key={child.href}
                        href={child.href}
                        className="block rounded-md px-3 py-2 text-sm text-neutral-700 hover:bg-brand-50 hover:text-brand-900"
                      >
                        {child.label}
                      </Link>
                    ))}
          ...
          <Container className="flex flex-col gap-1 py-3">
            {NAV_LINKS.map((link) => (
              <div key={link.label} className="border-b border-neutral-100 py-1">
                <Link
                  href={link.href}
                  className="block px-2 py-2 text-sm font-semibold text-brand-900"
